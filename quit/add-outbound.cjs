...
// 1. 获取出库单筛选选项
app.get('/api/outbound-orders/options', async (req, res) => {
  try {
...
// 4. 审核出库单 (扣减库存)
app.put('/api/outbound-orders/:id/audit', async (req, res) => {
  try {
    const { id } = req.params;
...
// 5. 撤销出库单
app.put('/api/outbound-orders/:id/revoke', async (req, res) => {
  try {
    const { id } = req.params;
...
// 6. 删除出库单
app.delete('/api/outbound-orders/:id', async (req, res) => {
  try {
    const { id } = req.params;
...
code = code.substring(0, insertPos) + outboundApis + code.substring(insertPos);
fs.writeFileSync(path, code);
console.log('Outbound APIs injected!');
