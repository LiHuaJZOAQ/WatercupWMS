...
const inventoryCards = ref([
  { value: 0, label: '原材料库存', trend: 'down', percentage: '常规', type: 'RawMaterial', status: 'good' },
  { value: 0, label: '成品库存', trend: 'up', percentage: '活跃', type: 'FinishedProduct', status: 'normal' }
])
...
const fetchDashboardData = async () => {
  try {
    ...
    if (data) {
      ...
      inventoryCards.value[0].value = invData.find(i => i.type === 'RawMaterial')?.totalQuantity || 0;
      inventoryCards.value[1].value = invData.find(i => i.type === 'FinishedProduct')?.totalQuantity || 0;
...
const viewDetails = (type) => {
  if (activeTab.value === 'inbound') {
    router.push({ path: '/inStorage/rawMaterial' })
